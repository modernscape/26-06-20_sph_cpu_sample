...
var _size$3 = /*@__PURE__*/ new Vector2();
/**
	...
	*/
	...
	updateBefore(frame) {
		...
		if (outputRenderTarget && outputRenderTarget.isXRRenderTarget === true) {
			...
			_size$3.set(outputRenderTarget.width, outputRenderTarget.height);
		} else {
			camera = this.camera;
			pixelRatio = renderer.getPixelRatio();
			renderer.getSize(_size$3);
		}
		this._pixelRatio = pixelRatio;
		this.setSize(_size$3.width, _size$3.height);
		const currentRenderTarget = renderer.getRenderTarget();
		const currentMRT = renderer.getMRT();
		const currentAutoClear = renderer.autoClear;
...
var ShadowNode = class extends ShadowBaseNode {
	...
	*/
	updateShadow(frame) {
		...
		_rendererState$1 = resetRendererAndSceneState(renderer, scene, _rendererState$1);
		...
		restoreRendererAndSceneState(renderer, scene, _rendererState$1);
	}
	/**
...
var TRAANode = class extends TempNode {
	...
	*/
	setViewOffset(width, height) {
		...
		this.camera.setViewOffset(viewOffset.fullWidth, viewOffset.fullHeight, viewOffset.offsetX + jitterOffset[0] - .5, viewOffset.offsetY + jitterOffset[1] - .5, viewOffset.width, viewOffset.height);
	}
	/**
	...
	*/
	setup(builder) {
		...
		if (builder.context.velocity !== void 0) this._velocityNode = builder.context.velocity;
		else this._velocityNode = velocity;
		...
		const samplePreviousDepth = (uv) => {
			...
			return this.camera.isOrthographicCamera ? viewZToOrthographicDepth(viewZ, this._cameraNearFar.x, this._cameraNearFar.y) : viewZToPerspectiveDepth(viewZ, this._cameraNearFar.x, this._cameraNearFar.y);
		};
		...
		const subpixelCorrection = Fn(([velocityUV, textureSize]) => {
			const phase = velocityUV.mul(textureSize).fract().abs();
			const weight = max(phase, phase.oneMinus());
			return weight.x.mul(weight.y).oneMinus().div(.75);
		}).setLayout({
		...
		const resolve = Fn(() => {
			...
			const currentWeight = float(.05).toVar();
			if (this.useSubpixelCorrection) currentWeight.addAssign(subpixelCorrection(offsetUV, textureSize).mul(.25));
			currentWeight.assign(hasValidHistory.select(currentWeight.add(motionFactor).saturate(), 1));
			return flickerReduction(currentColor, varianceClipping(positionTexel, currentColor, historyColor, mix(.5, 1, motionFactor.oneMinus().pow2())), currentWeight);
		});
...
* @returns {TRAANode}
*/
var traa = (beautyNode, depthNode, velocityNode, camera) => new TRAANode(convertToTexture(beautyNode), depthNode, velocityNode, camera);
//#endregion
export { TRAANode as default, traa };

//# sourceMappingURL=three_examples_jsm_tsl_display_TRAANode__js.js.map